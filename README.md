...
* Bitbucket requires a Java Developers Kit (JDK) or Java Run-time Environment (JRE) platform to be installed on your server's operating system. Oracle JDK / JRE (formerly Sun JDK / JRE)  version 8 and Open JDK/ JRE version 8 are currently supported by Atlassian.
...
You must have your database setup with the account user that Bitbucket will use. This can be done using the puppetlabs-postgresql and puppetlabs-mysql modules.
...
As RHEL 6 and its derivatives do not include a version of git that will work by default with bitbucket. We enable the repoforge module as a default if it is not already enabled. Whilst this is not best practice, it is better than the module not working for inexperienced users. By default we will upgrade git if it is already installed and the repoforge repository is not enabled. Default: true. You can turn all this functionality off with 'repoforge => false' and manage git outside of the module.
...
A complete example with postgres/nginx/bitbucket is available [here](https://github.com/mkrakowitzer/vagrant-puppet-bitbucket/blob/master/manifests/site.pp) or in the examples directory.
<a name="upgrades">
#####Upgrades
...
bitbucket::downloadURL:    'http://example.co.za/pub/software/development-tools/atlassian'
...
#####`git_manage`
Should bitbucket manage the git package. Can be 'true' or 'false', defaults to true.
#####`git_version`
The version of git to install. Default: 'installed'
#####`repoforge`
Enable the repoforge yum repository by default for RHEL as bitbucket requires a newer version of git.
